...
import { createContext, BLEND_MODES } from "nt-animator";

const BLUR_SCALING = 0.9;
...
function createShadow(source) {
	const width = source.width + SHADOW_PADDING;
	const height = source.height + SHADOW_PADDING;
	
	// next, render the car shadow
	shadowContainer.addChild(source);
	
	// center into the view
	// shadowContainer.width = source.width + (SHADOW_BLUR * 2);
	// shadowContainer.height = source.height + (SHADOW_BLUR * 2);
